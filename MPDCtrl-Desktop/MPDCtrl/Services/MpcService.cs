    ...
    */
    ...
    private async void MpdIdle()
    {
        ...
        try
        {
            ...
            if (isErr)
            {
                MpdAckError?.Invoke(this, errText);
                //ret.IsSuccess = false;
    ...
    private async Task<CommandResult> MpdCommandSendCommandProtected(string cmd, bool isAutoIdling = false)
    {
        // TMP:
        isAutoIdling = false;
            ...
            else
            {
                ...
                if (newCon.IsSuccess)
                {
                    ...
                    if (d.IsSuccess)
                    {
                        ...
                        ret = await MpdCommandSendCommandProtected(cmd, isAutoIdling);
                        //}
                    }
                }
                else
        ...
        catch (System.IO.IOException e)
        {
            ...
            if ((ConnectionState == ConnectionStatus.Disconnecting) || (ConnectionState == ConnectionStatus.DisconnectedByUser))
            {
                Debug.WriteLine("IOException@MpdSendCommand: ConnectionState Disconnecting" + " ReadLineAsync ---- " + e.Message);
            }
            else
            {
    ...
    private static async Task<CommandBinaryResult> MpdBinarySendBinaryCommandProtected(string cmd, AlbumCoverObject albumCover)
    {
        ...
        try
        {
            ...
            while (isWaitForOK)
            {
                ...
                {
                    while ((readSize = await _commandReader.BaseStream.ReadAsync(buffer)) > 0)
                    {
            ...
            else
            {
                if (isAck)
                {
                    ...
                    ret.ErrorMessage = ackText + " (@MpdBinarySendBinaryCommand): " + cmd;
                    ret.IsSuccess = false;
                    return ret;
                }
                ...
                {
                    ret.ErrorMessage = "No binary data(size) found. Could be AlbumArt instead of readpicture command? (@MpdBinarySendBinaryCommand)";
                    ret.IsSuccess = false;
                    return ret;
                }
            }
    ...
    public async Task<CommandImageResult> MpdQueryAlbumArt(string uri, bool isUsingReadpicture)
    {
        ...
        var albm = new AlbumCoverObject();
        albm.SongFilePath = uri;
        albm.IsDownloading = true;

        var escUri = Regex.Escape(uri);

        string cmd = "albumart \"" + escUri + "\" 0" + "\n";
        if (isUsingReadpicture && (!string.IsNullOrEmpty(MpdVerText)))
            if (CompareVersionString(MpdVerText, "0.22.0") >= 0)
                cmd = "readpicture \"" + escUri + "\" 0" + "\n";

        CommandBinaryResult BinResult = await MpdBinarySendBinaryCommand(cmd, albm);

        CommandImageResult ret = new();
        bool r = false;

        if (BinResult.IsSuccess)
        {
            albm.BinaryData = BinResult.BinaryData;

            if ((BinResult.WholeSize != 0) && (BinResult.WholeSize == BinResult.ChunkSize))
            {
                if (albm.BinaryData is not null)
                {
                    ret.AlbumCover.AlbumImageSource = BitmaSourceFromByteArray(albm.BinaryData);
                }

                if (ret.AlbumCover.AlbumImageSource is not null)
                {
                    albm.IsSuccess = true;
                }
                else
            ...
            else
            {
                if ((BinResult.WholeSize != 0) && (BinResult.WholeSize > BinResult.ChunkSize))
                {
                    if (BinResult.IsSuccess && (albm.BinaryData is not null))
                    {
                        // TODO:
                        while ((BinResult.WholeSize > albm.BinaryData.Length) && BinResult.IsSuccess)
                        {
                            BinResult = await MpdReQueryAlbumArt(albm.SongFilePath, albm.BinaryData.Length, isUsingReadpicture, albm);

                            if (BinResult.IsSuccess && (BinResult.BinaryData is not null))
                                albm.BinaryData = CombineByteArray(albm.BinaryData, BinResult.BinaryData);
                            else
                                break;
                        }
        ...
        {
            ret.IsTimeOut = BinResult.IsTimeOut;

            if (ret.IsTimeOut)
            {
                Debug.WriteLine("Timeout. @MpdQueryAlbumArt");
                ...
                if ((ConnectionState == ConnectionStatus.Disconnecting) || (ConnectionState == ConnectionStatus.DisconnectedByUser))
                {
                    Debug.WriteLine("ConnectionStatus.Disconnecting MpdQueryAlbumArt");
                ...
                else
                {
                    ...
                    if (newCon.IsSuccess)
                    {
                        ...
                        if (d.IsSuccess)
                        {
                            Debug.WriteLine("Reconnecting Success. @MpdQueryAlbumArt");
                            DebugCommandOutput?.Invoke(this, string.Format("Reconnecting Success. @MpdQueryAlbumArt" + Environment.NewLine));
                    ...
                    {
                        Debug.WriteLine("Reconnecting NOT Success. @MpdQueryAlbumArt");
                    }
                }
        ...
        if (!r)
        {
            ret.ErrorMessage = BinResult.ErrorMessage;
        }
        ret.AlbumCover.SongFilePath = albm.SongFilePath;
        ret.AlbumCover.IsDownloading = false;
        ret.AlbumCover.IsSuccess = r;
    ...
    private async Task<CommandBinaryResult> MpdReQueryAlbumArt(string uri, int offset, bool isUsingReadpicture, AlbumCoverObject albumCover)
    {
        if (string.IsNullOrEmpty(uri))
        {
    ...
    private static CommandBinaryResult ParseAlbumImageData(byte[] data, AlbumCoverObject albumCover)
    {
        ...
        if (data.Length > 2000000) //2000000000
        {
            ...
            albumCover.IsDownloading = false;
            r.ErrorMessage = "Binary file size too big. @ParseAlbumImageData()";
            r.IsSuccess = false;
            return r;
        }
        ...
        if (string.IsNullOrEmpty(albumCover.SongFilePath))
        {
            ...
            albumCover.IsDownloading = false;
            r.ErrorMessage = "File path is not set. @ParseAlbumImageData()";
            r.IsSuccess = false;
            return r;
        }
