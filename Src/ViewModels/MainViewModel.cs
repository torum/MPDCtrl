...
public sealed partial class MainViewModel : ObservableObject
{
    ...
    public bool IsBusy
    {
        ...
        set
        {
            ...
            field = value;
            _dispatcherService.TryEnqueue(() => { OnPropertyChanged(); });
        }
    }
    ...
    private async void DoChangeVolume(object? sender, System.Timers.ElapsedEventArgs e)
    {
        ...
        {
            _ = e;
            Debug.WriteLine($"Exception @DoChangeVolume: {ex}");
        }
    ...
    public AlbumEx? SelectedAlbum
    {
        get; set
        {
            ...
            try
            {
                Task.Run(async () =>
                {
                    await GetAlbumSongsAsync(field);
                    _dispatcherService.TryEnqueue(() =>
                    {
            ...
            {
                _ = ex;
                Debug.WriteLine($"SelectedAlbum Exception: {ex}");
            }
    ...
    public ObservableCollection<AlbumEx>? VisibleItemsAlbumsEx
    {
        get; set
        {
            ...
            {
                _ = ex;
                Debug.WriteLine($"VisibleItemsAlbumsEx Exception: {ex}");
            }
        }
    } = [];
    ...
    public string ShortStatusWithMpdVersion
    {
        ...
        get
        {
            if (IsConnected)
            {
                var connected = "Settings_MPD_Connection_Status_IsConnected".GetLocalized();
            ...
            {
                var connecting = "Settings_MPD_Connection_Status_IsConnecting".GetLocalized();

                return CurrentProfile is not null ? $"{connecting} ({CurrentProfile.Name})..." : $"{connecting}...";
            }
            else
            {
                var notConnected = "Settings_MPD_Connection_Status_IsNotConnected".GetLocalized();
...
#pragma warning restore CA1822
    ...
    private void InitializeAndSubscribe()
    {
        ...
        SearchTagList =
            [
                new Models.SearchOption(SearchTags.Title, $"{"SearchTag_Title".GetLocalized()}"),
                new Models.SearchOption(SearchTags.Artist, $"{"SearchTag_Artist".GetLocalized()}"),
                new Models.SearchOption(SearchTags.Album, $"{"SearchTag_Album".GetLocalized()}"),
                new Models.SearchOption(SearchTags.Genre, $"{"SearchTag_Genre".GetLocalized()}"),
                new Models.SearchOption(SearchTags.Any, $"{"SearchTag_Any".GetLocalized()}")
            ];

        // needs to be here for _resourceLoader.
        SearchShikiList =
            [
                new Models.SearchWith(SearchShiki.Contains, $"{"Search_Shiki_Contains".GetLocalized()}"),
                new Models.SearchWith(SearchShiki.Equals, $"{"Search_Shiki_Equals".GetLocalized()}")
            ];
    }
        ...
        _dispatcherService.TryEnqueue(async () =>
        {
            ...
            if (Queue.Count > 0)
            {
                if (isSongChanged || isCurrentSongWasNull)
                {
                    ...
                    if (item is not null)
                    {
                        ...
                        if (!string.IsNullOrEmpty(CurrentSong.File))
                        {
                            ...
                            {
                                //Debug.WriteLine("MpdQueryAlbumArt @UpdateStatus");
                                ...
                                if ((res != null) && (CurrentSong is not null))
                                {
                                    if (res.IsSuccess && (res.AlbumCover?.SongFilePath != null) && (CurrentSong.File != null))
                                    {
                                        if (res.AlbumCover?.SongFilePath == CurrentSong.File)
                                        {
                                            ...
                                            {
                                                //var bin = res.AlbumCover.BinaryData;
                ...
                else
                {
                    ...
                    // Update SystemMediaTransportControls
                    //Debug.WriteLine($"SetSystemMediaTransportControls ButtonStateOnly @UpdateStatus() ");
                    SetSystemMediaTransportControlsButtonStateOnly(buttonState);
                }
            }
            else
            {
    ...
    private void UpdateCurrentQueue()
    {
        ...
        _dispatcherService.TryEnqueue(async () =>
        {
                    ...
                    if (curitem is not null)
                    {
                        ...
                        if (IsDownloadAlbumArt && CurrentSong.IsAlbumCoverNeedsUpdate)
                        {
                            ...
                            if ((res.AlbumCover.IsSuccess) && (!res.AlbumCover.IsDownloading) && (res.AlbumCover?.SongFilePath != null))
                            {
                                if (res.AlbumCover?.SongFilePath == CurrentSong.File)
                                {
                                    UpdateProgress?.Invoke(this, "[UI] Received AlbumArt...");
                                    AlbumCover = res.AlbumCover;
                                    //AlbumArtBitmapSource = AlbumCover.AlbumImageSource;
                                    AlbumArtBitmapSource = await BitmapSourceFromByteArrayAsync(res.AlbumCover.BinaryData);
                                    UpdateProgress?.Invoke(this, "[UI] Received AlbumArt...(Converting..)");
                                    CurrentSong.IsAlbumCoverNeedsUpdate = false;
            ...
            else
            {
                ...
                {
                    StatusBarMessage = "Exception@UpdateCurrentQueue2: " + ex.Message;
    ...
    private void SetSystemMediaTransportControlsButtonStateOnly(Windows.Media.MediaPlaybackStatus playbackStatus)
    {
        //Debug.WriteLine("SetSystemMediaTransportControlsButtonStateOnly");
        _dispatcherService.TryEnqueue(() =>
        {
    ...
    private static async Task<RandomAccessStreamReference> BitmapImageToRandomAccessStreamReferenceAsync(BitmapImage bitmapImage)
    {
        ...
        // WriteableBitmap has no public access to the stream source, so we
        // must re-encode it from the raw pixel buffer.
    ...
    private static async Task<RandomAccessStreamReference?> ToRandomAccessStreamReferenceAsync(byte[]? byteArray)
    {
        ...
        {
            Debug.WriteLine("byteArray is null @ToRandomAccessStreamReference");
            return null;
        }
    ...
    private void UpdateCommandStatus()
    {
        _dispatcherService.TryEnqueue(() =>
        {
            ...
            QueueAddToPlaylistCommand.NotifyCanExecuteChanged();

            QueueSelectedPlayCommand.NotifyCanExecuteChanged();
            QueueListviewSortByCommand.NotifyCanExecuteChanged();
            QueueListviewSortReverseCommand.NotifyCanExecuteChanged();

            QueueSelectedPlayCommand.NotifyCanExecuteChanged();
            QueueListviewAddSelectedItemsToPlaylistCommand.NotifyCanExecuteChanged();
            QueueListviewRemoveSelectedItemsCommand.NotifyCanExecuteChanged();
        ...
        _dispatcherService.TryEnqueue(async () =>
        {
            ...
            if (playlistDir is not null)
            {
                ...
                if (SelectedNodeMenu is NodeMenuPlaylistItem nmpli)
                {
                    ...
                    {
                        await GetPlaylistSongsAsync(nmpli);
            ...
            {
                await GoToRenamedPlaylistPageAsync(RenamedSelectPendingPlaylistName);
                RenamedSelectPendingPlaylistName = string.Empty;
            }
        });
    }

    private async Task GoToPlaylistPageAsync(NodeMenuPlaylistItem playlist)
    {
    ...
    private async Task GoToRenamedPlaylistPageAsync(string playlist)
    {
    ...
    private async Task GetFilesAsync(NodeMenuFiles filestNode)
    {
        // TODO: fix this
    ...
    private async Task GetAlbumSongsAsync(AlbumEx album)
    {
        await _dispatcherService.EnqueueAsync(async () =>
        {
            IsWorking = true;
    ...
    private async Task GetAlbumPicturesAsync(IEnumerable<object>? albumExItems)
    {
        ...
        await _dispatcherService.EnqueueAsync(async () =>
        {
            ...
            foreach (var item in albumExItems)
            {
                ...
                if (_cts.Token.IsCancellationRequested)
                {
                    Debug.WriteLine("IsCancellationRequested in foreach1 @GetAlbumPictures");
                    return;
                }
                ...
                else
                {
                    ...
                    foreach (var albumsong in sresult)
                    {
                        ...
                        if (_cts.Token.IsCancellationRequested)
                        {
                            Debug.WriteLine("IsCancellationRequested in foreach2 @GetAlbumPictures");
                            return;
                        }
    ...
    private static async Task<BitmapImage?> BitmapSourceFromByteArrayAsync(byte[]? buffer)
    {
        if (buffer == null)
        {
    ...
    private async void OnConnectionError(MpcService sender, string msg)
    {
        ...
        try
        {
            _dispatcherService.TryEnqueue(() =>
            {
                ConnectionStatusMessage = "ConnectionStatus_ConnectionError".GetLocalized() + ": " + msg;
                StatusBarMessage = ConnectionStatusMessage;

                InfoBarErrTitle = $"{"ConnectionStatus_ConnectionError".GetLocalized()}";
        ...
        {
            _ = e;
            Debug.WriteLine($"Exception @OnConnectionError: {e}");
        }
    }
    ...
    private async void OnConnectionStatusChanged(MpcService sender, MpcService.ConnectionStatus status)
    {
        ...
        {
            _ = e;
            Debug.WriteLine($"Exception @OnConnectionStatusChanged: {e}");
        }
    }
    ...
    private async void OnMpdAckError(MpcService sender, string ackMsg, string origin)
    {
        ...
        try
        {
            _dispatcherService.TryEnqueue(async () =>
            {
                ...
                {
                    InfoBarAckTitle = MpdVersion + " " + "MPD_CommandResponse".GetLocalized();
                }
                else if (origin.Equals("Idle", StringComparison.OrdinalIgnoreCase))
                {
                    InfoBarAckTitle = MpdVersion + " " + "MPD_IdleResponse".GetLocalized();
                }
                else
                {
        ...
        {
            _ = e;
            Debug.WriteLine($"Exception @OnMpdAckError: {e}");
        }
    }
    ...
    private async void OnMpdFatalError(MpcService sender, string errMsg, string origin)
    {
        ...
        try
        {
            ...
            _dispatcherService.TryEnqueue(async () =>
            {
                if (origin.Equals("Command", StringComparison.OrdinalIgnoreCase))
                {
                    InfoBarErrTitle = MpdVersion + " " + "MPD_CommandResponse".GetLocalized();
                }
                else if (origin.Equals("Idle", StringComparison.OrdinalIgnoreCase))
                {
                    InfoBarErrTitle = MpdVersion + " " + "MPD_IdleResponse".GetLocalized();
                }
                else
                {
                    InfoBarErrTitle = MpdVersion;
        ...
        {
            _ = e;
            Debug.WriteLine($"Exception @OnMpdFatalError: {e}");
        }
    }
    ...
    private void OnMpcProgress(MpcService sender, string msg)
    {
        try
        {
            // Not used but may be usefull for debug.
            /*
            _dispatcherService.TryEnqueue(() =>
            {
            ...
            UpdateProgress?.Invoke(this, msg);
            */
        }
        catch (Exception e)
        {
            _ = e;
            Debug.WriteLine($"Exception @OnMpcProgress: {e}");
        }
    }
    ...
    private void OnMpcIsBusy(MpcService sender, bool on)
    {
        ...
        {
            _ = e;
            Debug.WriteLine($"Exception @OnMpcIsBusy: {e}");
        }
    }
    ...
    private async void OnUpdateProgress(string msg)
    {
        ...
        {
            _ = e;
            Debug.WriteLine($"Exception @OnUpdateProgress: {e}");
        }
    }
    ...
    private bool PlayNextCanExecute()
    {
    ...
    private bool PlayPrevCanExecute()
    {
    ...
    private bool PlayStopCanExecute()
    {
        if (!_mpc.Commands.Contains("stop")) { return false; }
        return true;
    }
    ...
    private bool PlayPauseCanExecute()
    {
        if (!_mpc.Commands.Contains("pause")) { return false; }
        return true;
    }
    ...
    private bool SetVolumeCanExecute()
    {
        if (!_mpc.Commands.Contains("setvol")) { return false; }
        return true;
    }
    ...
    [RelayCommand]
    private void QueueFilterSelect(object obj)
    {
        if (Queue.Count <= 1)
            return;
        /*
    ...
    private async Task QueueListviewMoveUp(object obj)
    {
        if (obj is null) return;
    ...
    private bool QueueListviewMoveCanExecute()
    {
    ...
    [RelayCommand(CanExecute = nameof(QueueListviewMoveCanExecute))]
    private async Task QueueListviewMoveDown(object obj)
    {
        if (obj is null) return;
    ...
    [RelayCommand(CanExecute = nameof(QueueListviewMoveCanExecute))]
    private async Task QueueListviewMovePos(Dictionary<string, string> idToNewPos)
    {
    ...
    [RelayCommand(CanExecute = nameof(QueueListviewMoveCanExecute))]
    private async Task QueueListviewSortBy(object obj)
    {
        if (obj is null)
        {
            return;
    ...
    [RelayCommand(CanExecute = nameof(QueueListviewMoveCanExecute))]
    private async Task QueueListviewSortReverse()
    {
        if (Queue.Count <= 1) return;
    ...
    [RelayCommand]
    private void ScrollIntoNowPlaying()
    {
    ...
    private async Task QueueAddToPlaylist()
    {
        if (Queue.Count == 0) return;
...
#pragma warning restore IDE0305
    ...
    // TODO: On drag and drop ShellPage is directly accessing this method... hence public....
    ...
    private bool PlaylistAddCanExecute()
    {
    ...
    [RelayCommand(CanExecute = nameof(PlaylistAddCanExecute))]
    private async Task QueueListviewAddSelectedItemsToPlaylist(object obj) //(System.Collections.Generic.IList<object> obj) Not safe for AoT.
    {
        if (Queue.Count == 0) return;
    ...
    private async Task QueueListviewRemoveSelectedItems(object obj) // (System.Collections.Generic.IList<object> obj)
    {
        if (obj is null) return;
    ...
    private bool DeleteIdCanExecute()
    {
    ...
    [RelayCommand(CanExecute = nameof(AddToQueueCanExecute))]
    private async Task SongsAddToQueue(object obj)
    {
        if (obj is null) return;
    ...
    [RelayCommand(CanExecute = nameof(PlaylistAddCanExecute))]
    private async Task SongsAddToPlaylist(object obj)
    {
        if (obj is null) return;
    ...
    [RelayCommand(CanExecute = nameof(SongsPlayCanExecute))]
    private async Task SongsSelectedItemsPlay(object obj) //(System.Collections.Generic.IList<object> obj)
    {
        if (obj is null)
        {
            return;
    ...
    [RelayCommand(CanExecute = nameof(PlaylistAddCanExecute))]
    private async Task SongsAddSelectedItemsToPlaylist(object obj) //(System.Collections.Generic.IList<object> obj)
    {
        if (obj is null)
        {
            return;
    ...
    [RelayCommand(CanExecute = nameof(AddToQueueCanExecute))]
    private async Task SongsAddSelectedItemsToQueue(object obj) //(System.Collections.Generic.IList<object> obj)
    {
        if (obj is null)
        {
            return;
    ...
    [RelayCommand(CanExecute = nameof(AddToQueueCanExecute))]
    private async Task SongsAddAfterSelectedItemsToQueue(object obj) //(System.Collections.Generic.IList<object> obj)
    {
        if (obj is null)
        {
            return;
        ...
        */
        ...
        if (selectedList.Count == 1)
        {
            await _mpc.MpdAddAfter(selectedList[0]);
        }
        else if (selectedList.Count > 1)
        {
            await _mpc.MpdAddAfter(selectedList);
        }
    }
    ...
    [RelayCommand]
    private async Task AlbumFilterSelect(object obj)
    {
        if (Albums.Count <= 1)
            return;
    ...
    [RelayCommand]
    private async Task ListviewGoToAlbumPage(SongInfo song)
    {
        if (song is null)
        {
            return;
        ...
        if (string.IsNullOrEmpty(asdf.Trim()))
        {
            ...
            if (hoge is not null)
            {
                await GoToAlbumDetailsPage(hoge);
            }
        }
        else
        {
            foreach (var item in items)
            {
                ...
                if (item is not null)
                {
                    await GoToAlbumDetailsPage(item);
                }
                break;
            }
        }
    ...
    [RelayCommand]
    private async Task GoToAlbumDetailsPage(AlbumEx album)
    {
        await _dispatcherService.EnqueueAsync(async () =>
        {
            ...
            await GetAlbumPicturesAsync(tmpCollection);
        });
    }

    [RelayCommand]
    private async Task GoBackFromAlbumDetailsPage(AlbumEx album)
    {
        await _dispatcherService.EnqueueAsync(async () =>
        {
            //
    ...
    [RelayCommand]
    private void SelectedAlbumGoToArtistPage(AlbumEx album)
    {
        if (album is null)
        {
            return;
    ...
    [RelayCommand(CanExecute = nameof(SongsPlayCanExecute))]
    private async Task SelectedAlbumArtistPlayAll(object obj)
    {
        if (obj is null) return;
    ...
    [RelayCommand(CanExecute = nameof(AddToQueueCanExecute))]
    private async Task SelectedAlbumArtistAddToQueue(object obj)
    {
        if (obj is null) return;
        if (SelectedAlbumArtist is null) return;
    ...
    [RelayCommand(CanExecute = nameof(PlaylistAddCanExecute))]
    private async Task SelectedAlbumArtistAddToPlaylist(object obj)
    {
        if (obj is null) return;
        if (SelectedAlbumArtist is null) return;
    ...
    [RelayCommand(CanExecute = nameof(AddToQueueCanExecute))]
    private async Task SelectedDirectoryAddToQueue(object obj)
    {
        if (obj is null) return;
    ...
    [RelayCommand(CanExecute = nameof(PlaylistAddCanExecute))]
    private async Task SelectedDirectoryAddToPlaylist(object obj)
    {
        if (obj is null) return;
    ...
    [RelayCommand(CanExecute = nameof(SongsPlayCanExecute))]
    private async Task FilesListviewPlayThis(object obj)
    {
        if (obj is null) return;
    ...
    [RelayCommand(CanExecute = nameof(AddToQueueCanExecute))]
    private async Task FilesListviewAddThis(object obj)
    {
        if (obj is null) return;
    ...
    [RelayCommand(CanExecute = nameof(SongsPlayCanExecute))]
    private async Task FilesSelectedItemsPlay(object obj) //(System.Collections.Generic.IList<object> obj)
    {
        if (obj is null)
        {
            return;
    ...
    [RelayCommand(CanExecute = nameof(PlaylistAddCanExecute))]
    private async Task FilesAddSelectedItemsToPlaylist(object obj) //(System.Collections.Generic.IList<object> obj)
    {
        if (obj is null)
        {
            return;
    ...
    [RelayCommand(CanExecute = nameof(AddToQueueCanExecute))]
    private async Task FilesAddSelectedItemsToQueue(object obj) //(System.Collections.Generic.IList<object> obj)
    {
        if (obj is null)
        {
            return;
    ...
    [RelayCommand(CanExecute = nameof(AddToQueueCanExecute))]
    private async Task FilesAddAfterSelectedItemsToQueue(object obj) //(System.Collections.Generic.IList<object> obj)
    {
        if (obj is null)
        {
            return;
    ...
    [RelayCommand]
    private static void FilesCopyFilePath(object obj)
    {
        if (obj is null)
        {
    ...
    private bool ChangePlaylistCanExecute()
    {
        if (!_mpc.Commands.Contains("stop")) { return false; }
        if (!_mpc.Commands.Contains("clear")) { return false; }
    ...
    private bool LoadPlaylistCanExecute()
    {
        if (!_mpc.Commands.Contains("load")) { return false; }
        return true;
    }
    ...
    private async Task RenamePlaylist(string playlist)
    {
        ...
        if (CheckIfPlaylistExists(result.PlaylistName))
        {
            ...
            var resultHint = new ContentDialog()
            {
                ...
                Content = "Dialog_PlaylistNameAlreadyExists".GetLocalized(),//$"Playlist \"{plname}\" already exists.", //
    ...
    private bool RenamePlaylistCanExecute()
    {
    ...
    private bool CheckIfPlaylistExists(string playlistName)
    {
        bool match = false;
    ...
    private async Task RemovePlaylist(string playlist)
    {
        if (string.IsNullOrEmpty(SelectedPlaylistName))
        {
            return;
    ...
    private bool PlaylistRemoveCanExecute()
    {
    ...
    private async Task ClearPlaylist(string playlist)
    {
        if (string.IsNullOrEmpty(SelectedPlaylistName))
        {
            return;
    ...
    private bool PlaylistClearCanExecute()
    {
    ...
    private async Task PlaylistRemoveSelectedItem(object obj)
    {
        if (obj is null)
        {
    ...
    private bool PlaylistDeletePosCanExecute()
    {
        if (!_mpc.Commands.Contains("playlistdelete")) { return false; }
        return true;
    }

    [RelayCommand(CanExecute = nameof(PlaylistMovePosCanExecute))]
    private async Task PlaylistMovePos(Dictionary<string, string> posToNewPos)
    {
    ...
    private bool PlaylistMovePosCanExecute()
    {
    ...
    private void PlaylistRemoveDuplicates(string playlist)
    {
        if (string.IsNullOrEmpty(SelectedPlaylistName))
        {
            return;
    ...
    [RelayCommand]
    private void PlaylistSongFilterSelect(object obj)//
    {
        if (obj is null) return;
    ...
    private bool PlaylistRemoveDuplicatesCanExecute()
    {
        if (!_mpc.Commands.Contains("playlistclear")) { return false; }
        if (!_mpc.Commands.Contains("playlistadd")) { return false; }
        return true;
    ...
    private async Task ReConnectWithSelectedProfile()
    {
        ...
        {
            _ = ex;
            Debug.WriteLine($"Exception @ReConnectWithSelectedProfile {ex}");
        }
        /*
    ...
    [RelayCommand]
    private async Task ShowProfileEditDialog()
    {
    ...
    [RelayCommand]
    private void ShowProfileRemoveNoneDialog()
    {
        if (Profiles.Count <= 0)
        {
            return;
    ...
    [RelayCommand]
    private async Task ShowProfileAddDialog()
    {
    ...
    private async Task AudioOutputToggleEnable(object obj)
    {
        if (obj is null)
        {
    ...
    private bool AudioOutputToggleCanExecute()
    {
    ...
    private void SwitchTheme(ElementTheme? param)
    {
        ...
        if (main.Content is FrameworkElement rootElement)
        {
